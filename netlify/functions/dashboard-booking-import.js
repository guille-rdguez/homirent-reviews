const { importBookingCsv } = require('./_booking_reviews');
const {
  json,
  methodNotAllowed,
  parseJsonBody,
...
exports.handler = async (event) => {
  ...
  const { session, error } = requireSession(event);
  ...
  try {
    const result = await importBookingCsv({
      propertyId,
      filename,
      csvBase64,
      uploadedBy: session?.username || 'dashboard',
    });
    return json(200, { ok: true, ...result });
  } catch (err) {
    console.error(err);
    return serverError(err.message || 'No se pudo importar el CSV de Booking');
  }
};
